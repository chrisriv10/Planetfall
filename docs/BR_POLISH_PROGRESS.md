...
The district/gameplay checkpoint was committed as `590af5b` (`Polish Battle Royale districts and harden gameplay interactions`). The next checkpoint, `41bfed2` (`Polish BR presentation and repair building access`), was committed and pushed to `origin/main` at the user's request on September 14. No manual deployment was performed or hosting result verified. The September 19 follow-up below is local and uncommitted.
This is a progress record, not a declaration that the full visual/gameplay acceptance standard has been met.

## September 20: stair wayfinding and E2E observation follow-up (local)

- Browser review of hotel stairs showed a usable but blank landing wall. Added five shallow residential landing markers across the current catalog, with floor numbers derived from actual upper landing slabs. Seven shared-material parts per marker (35 total) plus cached mounted signs; existing district LOD, no new colliders/lights or gameplay changes.
- The first screenshot exposed the markers buried behind the existing interior cladding. Corrected their mounting depth to sit in front of the 0.61m cladding projection, still within 0.76m of the wall center. Rechecked the hotel stair view on High and Low: `LEVEL 02` is visible and the route remains open. Nova street was also reviewed; its foreground remains dominated by the plaza sculpture and further composition work is still needed.
- Added a focused marker regression covering actual landing association, floor numbering, finite/bounded detail, visible-cladding clearance, doors and ceilings. Residential suite **7/7 passed**. Hotel stairs sampled 107 calls / 134,886 triangles / 21 textures on High; Low 86 calls / 101,532 triangles / 21 textures. 201 world materials; enabled instance counts 28,219 / 15,041. Background browser throttling was visible, so these are draw-counter samples, not useful FPS benchmarks.
- Corrected E2E `takeControl()` to foreground a page before checking pointer lock (a held lock did not guarantee foreground rendering). Navigation now reports if results have already arrived and releases W in a `finally` block. Started shove displacement assertions concurrently with acknowledgement instead of after it; the >0.6m / 3-second requirements are unchanged. These are test-observation corrections, not established fixes for all intermittent failures.
- An isolated run after the foreground change failed authoritative shove displacement (0.192m). The subsequent concurrent-observation run passed shove/return launch, but its saved failure was again the temporary cannon hit-feed message; results showed one 14-damage hit. Full E2E is not yet certified clean. No test was skipped and no gameplay value, test timeout or movement-distance threshold was relaxed.
- Fresh typecheck/build passed (BR 127.94 KB / 39.71 KB gzip, Classic unchanged). Full suite: **222 passed / 2 failed, 224 total across 34 files in 117.22s**. Failures were the 15-second wall-clock assertions in seeded 40-player Solo (16,959ms) and Duo (15,331ms), after valid completion/state checks had passed. Standalone rerun reproduced the budget failures (16,603ms / 18,391ms); Squad and deterministic replay passed. No authoritative server/shared changes exist in this continuation, and no performance threshold was relaxed. This is not a green validation result or proof of target-hardware performance. E2E is running separately because the unit failures stopped the chained command.
- Temporary art-review browser and preview server have been closed/stopped.

## September 19: residential interior follow-up (local)

- Completed the pending check-in/parcel-panel variation: shallow hotel check-in consoles and apartment parcel compartments replace blank space between wall seating. Shared bay-clearance checks skip doors, dividers and loot. Neither is a new gameplay interaction or collider.
- Added restrained ceiling panels, ribs and warm emissive strips derived from real overhead slabs. The pieces stay within the slabs, project less than 0.13m below them and do not bridge stair openings. Replaced the generic residential ceiling-light path to avoid duplicate fixtures. Existing instanced batches, materials and district LOD handle the additions; no extra dynamic lights.
- Computer-use screenshot review covered the hotel lounge before/after ceiling changes, Central Heights housing, and hotel stairs on Low. Found a lounge sign occluded by a corner column; recentered it above the seating bay and narrowed it from 3.4m to 3.1m. A subsequent screenshot showed the complete label.
- Added a dev-only Housing lounge review camera. Added focused tests for check-in versus parcels, west-door exclusion, ceiling/slab containment and sign corner/ceiling clearance. Focused residential tests: 6/6 passed. September 20 full-suite rerun: **223/223 across 34 files passed in 100.17s**, including BR lifecycle/network/performance coverage and Classic/Chaos regressions. The interrupted September 19 full-run result was not retained and is not counted as passed.
- Render-counter samples: hotel lounge 204 calls / 271,286 triangles / 27 loaded textures; housing 344 calls / 405,732 triangles / 41 textures; Low hotel stairs 97 calls / 103,364 triangles / 43 textures. Registry: 201 world materials. Enabled instances varied by district/preset (28,184 / 27,862 / 15,013). These include the live world and changing bots, not just these furnishings. The hidden browser was throttled to ~1 FPS, so no useful GPU/FPS comparison follows.
- Collision, authoritative simulation, weapons, movement, camera tuning, economy and Classic rendering are unchanged. The broad room shapes and generic wall composition still need improvement; these are screenshot-reviewed incremental changes, not completion of the whole art/gameplay acceptance pass.
- Follow-up accessibility correction: the inactive Classic `CHAOS ROUND` banner appeared in the browser accessibility tree during BR because opacity alone hid it. Inactive reveal banners now also use `visibility: hidden`; the active Chaos reveal explicitly restores visibility. E2E now checks the BR banner is hidden and the active Chaos reveal is visible. This does not change gameplay, banner duration or mode selection.
- September 20 typecheck/build passed. BR chunk 126.70 KB / 39.44 KB gzip; Classic chunk unchanged at 445.40 KB / 112.35 KB gzip. Existing shared Three/Rapier large-chunk warning remains.
- Fresh E2E initially finished **8/9 in 3.6 minutes**: the Classic raid timed out waiting for return-launch arrival. An isolated rerun completed that launch and both cannon hits (28 damage in results), then failed checking a temporary hit-feed entry after the second-shot sequence. Feed entries expire after 6.2 seconds. Moved the same feed assertion alongside the first hit and retained both authoritative shot/damage checks. Added an explicit return-launch event assertion to distinguish server acceptance from arrival on future failures. No gameplay values, test distance thresholds or timeouts were changed.
- Final full E2E rerun: **8/9 passed in 6.7 minutes**, **not green**. The raid exceeded its 220-second test timeout inside `moveTo(guest, cannon...)`, after the return-launch event and arrival checks passed. The captured page had reached Draw/results with no cannon damage. Settings, shop, BR drop/landing/isolation, cannon impact marker, multiplayer startup, Chaos visibility, six participants and solo bots all passed. This was actual Playwright execution, not a spawn/EPERM failure. The earlier return-launch failure's cause is still unestablished; the isolated and final runs did not reproduce it. Slow navigation/background rendering versus a live 90-second test match needs further diagnosis before claiming the complete E2E regression clean. No full human combat/controller or stable target-hardware FPS validation was performed in this continuation.
...
`main` remains at the pushed checkpoint `41bfed2`; the September 19 continuation described at the top is uncommitted. No further push or deployment was performed.
