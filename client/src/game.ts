...
export class PlanetfallGame {
  ...
  private makePlayer(state: PlayerState): PlayerVisual {
    ...
    const astronaut = createAstronautVisual({ identityColor: state.color, suitAccent, isBot: state.isBot });
    const { group, torso, helmet, visor, backpack, leftArm, rightArm, leftLeg, rightLeg, suitMaterial: suit, lodDetails } = astronaut;
